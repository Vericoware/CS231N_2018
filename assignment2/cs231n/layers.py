...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    x_net = x.reshape(x.shape[0],-1)
    out = np.dot(x_net, w)+b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    dx = dout.dot(cache[1].T)
    dx = dx.reshape(cache[0].shape)
    x_net = cache[0].reshape(cache[0].shape[0],-1)
    dw = x_net.T.dot(dout)
  ...
  #compute out[i,f,h,w]
    for i in range(N):
        for f in range(F):
            for h in range(H_):
                for j in range(W_):                    
                    out[i,f,h,j] = np.sum(x_pad[i,:,h*s:h*s+HH,j*s:j*s+WW]*w[f])+b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
    ...
    for i in range(N):
        for f in range(F):
            for h in range(H_):
                for j in range(W_):
                    db[f] += dout[i,f,h,j]
                    dw[f] += dout[i,f,h,j]*x_pad[i,:,h*s:h*s+HH,j*s:j*s+WW]
                    dx_pad[i,:,h*s:h*s+HH,j*s:j*s+WW] += dout[i,f,h,j]*w[f]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for c in range(C):
            for j in range(H_):
                for k in range(W_):
                    temp1 = x[i,c,j*s:j*s+ph,k*s:k*s+pw]
                    m = np.max(temp1)
                    temp2 = np.zeros(temp1.shape)
                    temp2[temp1==m] = 1
                    dx[i,c,j*s:j*s+ph,k*s:k*s+pw] = dout[i,c,j,k] * temp2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
